var events='focus blur click resize scroll cut copy paste keydown keyup keypress mouseenter mouseover mousemove mousedown mouseup dblclick contextmenu wheel mouseleave mouseout select dragstart drag dragend dragenter dragover dragleave drop fullscreenchange beforeprint afterprint pagehide pageshow popstate cached unload beforeunload load online offline open close'.split(' '),
...
for (var i=0;i<events.length;i++) {
  ...
  document.addEventListener(events[i],(e)=>{
    ...
    if (!'mousemove drag dragover'.split(' ').includes(e.type)) {
