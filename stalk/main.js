(function() {
  ...
  document.querySelector('people').onclick=e=>{
    if (e.target.tagName==='PERSON') {
      ...
      document.querySelector('#alias').innerHTML=data.aliases.join(', ');
      s='';
      for (var i=0;i<data.ships.length;i+=2) s+=data.ships[i]+' ('+data.ships[i+1]+'), ';
      document.querySelector('#ship').innerHTML=s.slice(0,-2);
      s='';
      ...
      document.querySelector('#edu').innerHTML=s.slice(0,-2);
      document.querySelector('#org').innerHTML=data.organisations.join(', ');
      s='';
      for (var i=0;i<data.profiles.length;i+=2) s+='<a href="'+data.profiles[i+1]+'">'+data.profiles[i]+'</a> ';
      document.querySelector('#profs').innerHTML=s.slice(0,-1);
